...
export const fancyButtonVariants = tv({
  ...
  variants: {
    variant: {
      neutral: { root: ['bg-foreground text-background', 'shadow-sm border border-foreground/10'] },
      primary: { root: ['bg-primary text-primary-foreground', 'shadow-sm border border-primary/10'] },
      destructive: { root: ['bg-destructive text-destructive-foreground', 'shadow-sm border border-destructive/10'] },
      basic: { root: ['bg-background text-foreground border border-border/30', 'shadow-sm', 'hover:bg-accent hover:text-accent-foreground hover:border-border/50'] },
      ghost: { root: ['text-foreground', 'hover:bg-accent hover:text-accent-foreground'] },
      link: { root: ['text-primary underline-offset-4', 'hover:underline'] },
    },
    size: {
      default: { root: 'h-10 px-4 py-2 gap-2 rounded-xl', icon: 'size-4' },
      sm: { root: 'h-9 px-3 gap-2 text-sm rounded-xl', icon: 'size-3.5' },
      lg: { root: 'h-11 px-8 gap-2 rounded-xl', icon: 'size-4' },
      icon: { root: 'h-10 w-10 rounded-xl', icon: 'size-4' },
    },
  },
  compoundVariants: [
    {
...
const FancyButtonRoot = React.forwardRef<HTMLButtonElement, FancyButtonProps>(
  ({ asChild, children, variant, size, className, ...rest }, forwardedRef) => {
    ...
    const sharedProps: FancyButtonSharedProps = { variant, size }
...
function FancyButtonIcon<T extends React.ElementType>({
  className, variant, size, as, ...rest
