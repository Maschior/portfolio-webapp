...
interface NavigationDockProps {
  items: Array<{ name: string; href: string; icon: React.ComponentType<{ className?: string }>; isActive: boolean }>
  className?: string
}
...
export function NavigationDock({ items, className }: NavigationDockProps) {
  ...
  const handleLinkClick = (href: string, isActive: boolean, event: React.MouseEvent) => {
          ...
          return (
            ...
            >
              <FancyButton.Root
                ...
                className={cn('relative overflow-hidden w-full', prefersReducedMotion ? 'transition-none' : 'transition-all duration-500 ease-out')}
              >
                <Link
                  href={item.href}
                  prefetch={true}
                ...
                >
                  ...
                  <span
                    className={cn(
                      ...
                      prefersReducedMotion ? 'transition-none' : 'transition-[max-width,margin] duration-500 ease-out',
                      isActive ? 'opacity-100 max-w-[80px] ml-2 relative' : 'opacity-0 max-w-0 ml-0 absolute'
                    )}
                    style={{ opacity: isActive ? 1 : 0, transition: prefersReducedMotion ? 'none' : 'max-width 500ms ease-out, margin 500ms ease-out' }}
                  >
                    {item.name}
                  </span>
                  {isActive && (
              ...
              {!isActive && (
                <div
                  className={cn(
                    ...
                    prefersReducedMotion ? 'transition-none' : 'transition-opacity duration-200'
                  )}
                  role="tooltip"
                >
                  {item.name}
